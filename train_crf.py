...
if os.path.isfile(os.path.join(asset_path, ckpt_path, load_ckpt_file_name % load_restore_epoch)):
    checkpoint = torch.load(os.path.join(asset_path, ckpt_path, load_ckpt_file_name % load_restore_epoch), , map_location=torch.device('cpu'))
...
if os.path.isfile(os.path.join(asset_path, ckpt_path, ckpt_file_name % restore_epoch)):
    checkpoint = torch.load(os.path.join(asset_path, ckpt_path, ckpt_file_name % restore_epoch), , map_location=torch.device('cpu'))
...
if os.path.exists(z_path):
    normalization = torch.load(z_path, , map_location=torch.device('cpu'))
...
if os.path.isfile(os.path.join(asset_path, ckpt_path, ckpt_file_name % last_best_epoch)):
    checkpoint = torch.load(os.path.join(asset_path, ckpt_path, ckpt_file_name % last_best_epoch), map_location=torch.device('cpu'))
    crf.load_state_dict(checkpoint['model'])

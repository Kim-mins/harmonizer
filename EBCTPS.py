...
def enframe(x, win, inc):
  ...
  a = np.array([[indf[i]]*lw for i in range(len(indf))],
               dtype='int64') + np.array(([list(inds)]*nf), dtype='int64')-1
  f = x[a]
...
def ETS(x, Fs, scaling_factor, zff_gci):
  ...
  if scaling_factor == 1:
    ...
    zff_array = np.concatenate(
        (np.zeros(shift_val), zff_array, np.zeros(Kmax+shift_val)), axis=0)
    x = np.concatenate(
        (np.zeros(shift_val), x, np.zeros(Kmax+shift_val)), axis=0)
    ...
    for y_last in range(shift_val, output_len - frame_len + 1, Ss):
      ...
      mat_ind_Sa = [j for j in range(len(
          zff_array[search_strt_pt_Sa-1: search_end_pt_Sa])) if zff_array[j+search_strt_pt_Sa-1] > 0]
      mat_ind_y = [j for j in range(len(
          zff_array_y[search_strt_pt_y-1: search_end_pt_y])) if zff_array_y[j+search_strt_pt_y-1]]
      ...
      if mat_ind_Sa == [] or mat_ind_y == [] or (mat_ind_Sa[0] - mat_ind_y[0]) > Kmax:
        ...
        valid_ind = [j for j in range(
            len(mat_ind_Sa)) if mat_ind_Sa[j] > mat_ind_y[0]]
      ...
      x_ovrlp = np.array(range(
          search_strt_pt_Sa+km[i-1], search_strt_pt_Sa+shift_val+km[i-1]), dtype='int64')
      x_nonovrlp = np.array(range(
          search_strt_pt_Sa+km[i-1]+shift_val, search_end_pt_Sa+km[i-1]+1), dtype='int64')

      zff_array_y[y_ovrlp-1] = zff_array_y[y_ovrlp-1] + zff_array[x_ovrlp-1]
      zff_array_y[y_nonovrlp-1] = zff_array[x_nonovrlp-1]

      scaled_sp[y_ovrlp-1] = scaled_sp[y_ovrlp-1] * \
          hamming_win[shift_val:] + x[x_ovrlp-1]*hamming_win[: shift_val]
...
def EPS(x, Fs, scaling_factor, zff_gci):
  ...
  if scaling_factor == 1:
    ...
    scaled_sp = resample(time_scaled_sp, int(
        len(time_scaled_sp)/scaling_factor))
